...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//	glm::mat4 model;
		//	model = glm::translate(model, glm::vec3(0.0f, 0.0f, -10.0f*i));
		...
		{
			for (GLuint j = 0; j < 21; ++j){
				glm::mat4 model;
				if (pMaze->m_arrMaze[i][j]._state == 1){

					model = glm::translate(model, glm::vec3((GLfloat)pMaze->m_arrMaze[i][j]._x,(GLfloat)pMaze->m_arrMaze[i][j]._y,0.0f));
					//model = glm::translate(model, glm::vec3((GLfloat)pMaze->m_arrMaze[i][j]._x,0.0f, (GLfloat)pMaze->m_arrMaze[i][j]._y));
					GLuint modelLoc = glGetUniformLocation(ourShader.Program, "model");
					glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
